// ══ CONFIGURATION BASE64 NTFY (ANTI-BLOCAGE) ══════════════════
let ROOM_CODE = localStorage.getItem('musabaka_room_code') || "";
const SYNC_BASE = "https://ntfy.sh/musabaka_adl_v7_"; // Canal V7
...
// ══ SYNC CORE ══
// Encodage Base64 pour éviter toute corruption ou blocage réseau
...
async function saveRoomState() {
  ...
  try {
    const payload = encodeState(syncState);
    await fetch(SYNC_BASE + ROOM_CODE, {
      method: 'POST',
      body: payload
    });
    logDebug("Données envoyées ✅");
  } catch (e) { 
    logDebug("Erreur d'envoi ❌"); 
    if(app.role === 'supervisor') alert("Erreur: Le réseau bloque l'envoi.");
  }
}
...
function startSyncListener() {
  ...
  app.eventSource = new EventSource(SYNC_BASE + ROOM_CODE + "/sse");
  
  app.eventSource.onopen = () => {
    updateSyncStatus('online', "متصل ✅");
    logDebug("Canal Ouvert 📡");
  };
  ...
  app.eventSource.onmessage = (e) => {
    try {
      ...
      applyState(decodeState(envelope.message));
      logDebug("Mise à jour reçue ⚡");
    } catch (err) {}
  };
...
async function forceSync() {
  ...
  try {
    const res = await fetch(SYNC_BASE + ROOM_CODE + "/json?poll=1");
    const text = await res.text();
    if (!text) {
      logDebug("Rien sur le serveur");
      return;
    }
    const lines = text.trim().split('\n');
    const lastMsg = JSON.parse(lines[lines.length - 1]);
    applyState(decodeState(lastMsg.message));
    logDebug("Synchro OK");
    updateSyncStatus('online', "متصل ✅");
  } catch(e) { logDebug("Synchro retardée"); }
}
