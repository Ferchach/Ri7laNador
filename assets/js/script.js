...
async function initRoom() {
  ...
  app.pollInt = setInterval(loadRoomState, 1500); // Shorter poll for faster sync
}
...
function startListening() {
  ...
  app.eventSource.onmessage = (e) => {
    try {
      ...
      if (data.type === 'ANSWER_SUBMIT' && app.role === 'supervisor') {
...
async function launchQuestion() {
  ...
  app.activeQuestion = {
    ...
    qKey: app.currentCat + "_" + app.currentQIdx // Unique ID
  };

  await saveRoomState();
  publishState({ type: 'QUESTION_LAUNCH' });
...
function syncParticipantState() {
  ...
  if (app.activeQuestion) {
    // FORCE CHANGE if qKey is different, even if q-view is visible
