// ══ REAL-TIME SYNC (via HTTP KV Store) ══════════════════════
// Using kvdb.io which works over standard HTTPS (very robust)
...
async function apiPut(key, val) {
  try {
    await fetch(BUCKET_URL + key, {
      method: 'POST', // kvdb uses POST/PUT to set
      body: typeof val === 'object' ? JSON.stringify(val) : val
    });
    return true;
  } catch (e) { console.error("API Put Error:", e); return false; }
}

async function apiGet(key) {
  try {
    const r = await fetch(BUCKET_URL + key + '?' + Date.now()); // cache-buster
    if (!r.ok) return null;
...
function updateSyncStatus(isOnline, msg) {
  ...
  if (isOnline) {
    ...
    txt.textContent = msg || "متصل بالخدمة";
  } else {
...
function startSupervisorSync() {
  updateSyncStatus(true, "المشرف متصل");
  clearInterval(app.syncInt);
  app.syncInt = setInterval(async () => {
    // Poll for answers from teams 1-20
    for (let i = 1; i <= 20; i++) {
...
function startParticipantSync() {
  updateSyncStatus(true, "المشارك متصل");
  clearInterval(app.syncInt);
  app.syncInt = setInterval(async () => {
    const data = await apiGet('state');
    if (data && data.active && data._v !== lastV) {
      lastV = data._v;
