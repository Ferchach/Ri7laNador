...
const COMP_ID = "musabaka_cultural_adl_2026_room1";

// Using more diverse and stable public relay peers
const PEERS = [
...
const gun = Gun(PEERS);
const db = gun.get(COMP_ID);

// Connection monitor
gun.on('hi', peer => {
  console.log("Connected to peer:", peer.url);
  document.getElementById('sync-dot').classList.add('online');
  document.getElementById('sync-text').textContent = "متصل بالشبكة";
});

gun.on('bye', peer => {
  console.log("Disconnected from peer:", peer.url);
  // Only show offline if NO peers are left (simplified)
  setTimeout(() => {
    if (document.querySelectorAll('.online').length === 0) {
      document.getElementById('sync-dot').classList.remove('online');
      document.getElementById('sync-text').textContent = "جاري إعادة الاتصال...";
    }
  }, 2000);
});
...
function listenForAnswers() {
  console.log("Supervisor listening for answers...");
  db.get('answers').map().on((ans, tn) => {
    if (ans) {
      console.log("Answer received from team", tn, ":", ans);
      shared.answers[tn] = ans;
      renderAnswers();
    }
  });
}
...
function launchQuestion() {
  ...
  console.log("Launching question:", q.q);
  
  // Clear answers in Gun and local
  db.get('answers').put(null);
  shared.answers = {};
  ...
  db.get('state').put({
    ...
    qText: q.q,
    qType: q.type,
    opts: JSON.stringify(q.opts || (q.type === 'mimes' ? q.words : [])),
    tStart: now,
    dur: dur,
    _v: now // force update timestamp
  });
  ...
  app.supTimerInt = setInterval(() => {
    ...
    if (rem <= 0) {
      ...
      db.get('state').get('active').put(false);
...
function nextQ() {
  const qs = QUESTIONS[app.currentCat];
  db.get('state').put({ active: false, _v: Date.now() });
  clearInterval(app.supTimerInt);
...
function startRealTimeSync() {
  console.log("Participant starting real-time sync...");
  db.get('state').on(data => {
    ...
    if (data && data.active) {
      ...
      shared.opts = JSON.parse(data.opts || '[]');
...
function sendAnswer() {
  ...
  console.log("Sending answer to Gun:", ans);
  db.get('answers').get(app.teamNum).put(ans);
...
function resetAll() {
  Object.assign(app, {
    ...
    partTimerInt: null, pollInt: null, selectedOpt: null
  });
