...
let NTFY_URL = "";
const BUCKET_URL = "https://kvdb.io/mn6xR6f8XzY9vP7u5nB2/"; // Persistent KV Store
...
async function loadRoomState() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      logDebug("State restored from cloud.");
    }
  } catch (e) { logDebug("No existing state found."); }
}
...
const app = {
  ...
  completedCats: [], supTimerInt: null, partTimerInt: null, eventSource: null, selectedOpt: null
};
...
async function initRoom() {
  ...
  logDebug("Initializing Room: " + ROOM_CODE);
  await loadRoomState(); // Sync scores if already exists
  goto('screen-role');
  startListening();
}
...
function startListening() {
  ...
  app.eventSource.onopen = () => {
    updateSyncStatus('online', "جاهز - غرفة " + ROOM_CODE);
  };
...
async function sendAnswer() {
  ...
  let ans = (qType === 'mcq' || qType === 'mimes') ? (app.selectedOpt || "(بدون إجابة)") : ($('part-open-inp').value.trim() || "(بدون إجابة)");
...
function exportToSheets() {
  let csv = "Team Number,Team Name,Score\n";
  Object.entries(app.teams).forEach(([tn, data]) => {
    csv += `${tn},${data.name},${data.score}\n`;
  });
  ...
  a.setAttribute('download', `results_room_${ROOM_CODE}.csv`);
...
function confirmReset() {
  if (confirm("هل أنت متأكد من مسح جميع النتائج والبدء من جديد؟")) {
    resetCompetition();
  }
}
