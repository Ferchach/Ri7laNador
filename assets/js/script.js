...
async function saveRoomState() {
  ...
  try {
    // On publie l'état sur ntfy
    await fetch(SYNC_BASE + ROOM_CODE, {
      method: 'POST',
      body: JSON.stringify(state)
    });
    logDebug("تم الحفظ ✅");
  } catch (e) { logDebug("خطأ في الحفظ"); }
}

// Nouvelle fonction de réception en temps réel (SSE)
function startSyncListener() {
  if (app.eventSource) app.eventSource.close();
  
  // On demande à ntfy de nous envoyer les messages en direct
  app.eventSource = new EventSource(SYNC_BASE + ROOM_CODE + "/sse");
  
  app.eventSource.onopen = () => {
    updateSyncStatus('online', "متصل الآن ✅");
    logDebug("Connection Open");
    // Au démarrage, on demande l'état actuel une fois
    loadInitialState();
  };
  ...
  app.eventSource.onmessage = (e) => {
    try {
      ...
      // Mise à jour de l'application avec les données reçues
