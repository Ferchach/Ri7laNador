// ══ REAL-TIME SYNC (Robust HTTP) ═════════════════════════════
// This bucket ID is unique for your competition
const BUCKET_ID = "adl_musabaka_room_99"; 
const BUCKET_URL = "https://kvdb.io/K9m8vH7vXjY9rP4m8vH7vX/"; // Public stable bucket
...
async function apiPut(key, val) {
  try {
    ...
    console.error("Sync Error (Put):", e); 
    return false; 
  }
}
...
async function apiGet(key) {
  try {
    const res = await fetch(BUCKET_URL + key + "?t=" + Date.now(), { cache: 'no-cache' });
...
async function initSync() {
  ...
  if (ok) {
    updateSyncStatus('online', "جاهز للمزامنة");
