// ══ CONFIGURATION HYBRIDE (ANTI-BLOCAGE) ════════════════════════
let ROOM_CODE = localStorage.getItem('musabaka_room_code') || "";
const KVDB_BUCKET = "https://kvdb.io/mn6xR6f8XzY9vP7u5nB2/"; // Stockage principal
const NTFY_TOPIC = "https://ntfy.sh/musabaka_adl_ping_v6_"; // Signal de réveil

function logDebug(msg) {
  const area = document.getElementById('debug-area');
  if (area) {
    const time = new Date().toLocaleTimeString();
    area.innerHTML = `[${time}] ${msg}<br>` + area.innerHTML.substring(0, 200);
  }
}
...
async function saveRoomState() {
  ...
  try {
    // 1. Sauvegarde silencieuse sur KVDB
    await fetch(KVDB_BUCKET + "room_" + ROOM_CODE, {
      method: 'PUT',
      body: JSON.stringify(state)
    });
    // 2. Envoi du signal de réveil
    fetch(NTFY_TOPIC + ROOM_CODE, { method: 'POST', body: "WAKEUP" }).catch(()=>{});
    logDebug("Sauvegarde KVDB OK ✅");
  } catch (e) { logDebug("Erreur KVDB ❌"); }
}

async function forceSync() {
  if (!ROOM_CODE) return;
  logDebug("Lecture KVDB...");
  try {
    const res = await fetch(KVDB_BUCKET + "room_" + ROOM_CODE + "?t=" + Date.now());
    if (res.ok) {
      const text = await res.text();
      if(text.length > 5) {
        const data = JSON.parse(text);
        applyState(data);
        updateSyncStatus('online', "متصل بالبيانات ✅");
      }
    } else {
      updateSyncStatus('offline', "غرفة جديدة (انتظر المشرف)");
    }
  } catch(e) {
    updateSyncStatus('offline', "خطأ في الشبكة");
  }
}
...
function startSyncListener() {
  ...
  // Écoute des signaux de réveil
  app.eventSource = new EventSource(NTFY_TOPIC + ROOM_CODE + "/sse");
  app.eventSource.onopen = () => logDebug("Écouteur Actif 📡");
  
  app.eventSource.onmessage = (e) => {
    try {
      const envelope = JSON.parse(e.data);
      if (envelope.message === "WAKEUP") {
        logDebug("Signal Reçu ⚡");
        forceSync(); // On va lire la base KVDB
      }
    } catch (err) {}
  };

  // Sécurité absolue : Polling KVDB toutes les 2 secondes (KVDB n'a pas de limite 429)
  clearInterval(app.pollInt);
  app.pollInt = setInterval(() => {
    if (app.role === 'participant' || app.role === 'jury') forceSync();
  }, 2000);
}
...
const app = {
  ...
  mimeWord: null, activeQuestion: null, lastQKey: null
};
...
async function initRoom() {
  ...
  startSyncListener(); 
  forceSync(); // On tente de récupérer la dernière question publiée
  goto('screen-role');
}
...
async function finishQuestionRound(q) {
  ...
  localStorage.setItem('musabaka_log', JSON.stringify(app.gameLog)); // Save locally
...
function runPartTimer(startMs, dur) {
  ...
  app.partTimerInt = setInterval(() => {
    ...
    if (rem <= 0) { clearInterval(app.partTimerInt); if (!$('part-send-btn').classList.contains('hidden')) sendAnswer("(انتهى الوقت)"); }
  }, 200);
}
...
async function confirmReset() {
  if (confirm("هل تريد حذف هذه الغرفة نهائياً من السيرفر؟ سيتم طرد جميع المشاركين.")) {
