...
function startSyncListener() {
  ...
  app.eventSource.onopen = () => {
    ...
    // On ne fait plus de poll automatique ici pour éviter l'erreur 429
  };
  ...
  app.eventSource.onmessage = (e) => {
    try {
      ...
      applyState(JSON.parse(envelope.message));
      logDebug("Sync Received");
    } catch (err) { }
  };
...
// Bouton de secours pour forcer la synchro
async function forceSync() {
  logDebug("Manual Sync...");
  try {
    ...
    applyState(JSON.parse(lastMsg.message));
    logDebug("Sync Done");
  } catch(e) { logDebug("Sync Failed (Rate Limit)"); }
}
...
const QUESTIONS = {
  ...
  "كرة القدم": [
    ...
    { q: "من هو اللاعب الذي سجل هدف الفوز للمغرب ضد البرتغال في مونديال قطر؟", type: "mcq", opts: ["حكيم زياش", "سفيان bofal", "يوسف النصيري", "أشرف حكيمي"], ans: "يوسف النصيري", pts: 2 }
  ],
  "الجغرافي": [
...
function joinTeam() {
  ...
  forceSync(); // On force une synchro au démarrage
}
