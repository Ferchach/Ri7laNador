...
// Utilisation d'un bucket KVDB dédié et unique
...
// ══ API CORE (KVDB ONLY FOR MAXIMUM RELIABILITY) ══
async function saveRoomState() {
  ...
  try {
    await fetch(BUCKET_URL + "musabaka_" + ROOM_CODE, {
      method: 'PUT',
      body: JSON.stringify(state)
    });
    logDebug("Cloud Synced ✅");
  } catch (e) { logDebug("Cloud Error ❌"); }
}

async function loadRoomState() {
  if (!ROOM_CODE) return;
  try {
    const res = await fetch(BUCKET_URL + "musabaka_" + ROOM_CODE + "?t=" + Date.now());
    if (res.ok) {
      ...
      updateSyncStatus('online', "نشط - غرفة " + ROOM_CODE);
      ...
      if (app.role === 'supervisor') renderScores();
       ...
       updateSyncStatus('offline', "غرفة جديدة");
    }
  } catch (e) { updateSyncStatus('offline', "خطأ اتصال"); }
}
...
async function initRoom() {
  ...
  // ULTRA-FAST POLLING (1.2s) - This replaces SSE for 100% reliability
  clearInterval(app.pollInt);
  app.pollInt = setInterval(loadRoomState, 1200); 
}
...
async function launchQuestion() {
  ...
  app.activeQuestion = {
    ...
    qKey: "Q_" + now // Unique key to force update
  };
...
async function confirmReset() {
  if (confirm("هل تريد حذف هذه الغرفة نهائياً من السيرفر؟ سيتم طرد جميع المشاركين.")) {
    try {
      await fetch(BUCKET_URL + "musabaka_" + ROOM_CODE, { method: 'DELETE' });
      logDebug("Room Deleted from Cloud");
    } catch(e) {}
    localStorage.clear();
    location.reload();
  }
