...
let NTFY_URL = "";
// Fresh Bucket ID for this session
const BUCKET_URL = "https://kvdb.io/v1_musabaka_adl_secure_99/"; 

function logDebug(msg) {
  const area = document.getElementById('debug-area');
  if (area) {
    const time = new Date().toLocaleTimeString();
    area.innerHTML = `[${time}] ${msg}<br>` + area.innerHTML.substring(0, 300);
...
async function saveRoomState() {
  ...
  try {
    await fetch(BUCKET_URL + ROOM_CODE, {
      method: 'PUT',
      body: JSON.stringify(state)
    });
  } catch (e) { logDebug("Cloud Save Error"); }
}

async function loadRoomState() {
  if (!ROOM_CODE) return;
  try {
    const res = await fetch(BUCKET_URL + ROOM_CODE + "?t=" + Date.now());
    if (res.ok) {
...
const app = {
  ...
  mimeWord: null, activeQuestion: null, pollInt: null, lastQKey: null
};
  ...
  app.eventSource.onmessage = (e) => {
    try {
      ...
      if (data.type === 'QUESTION_LAUNCH') {
        logDebug("Launch signal received");
        if (data.activeQuestion) {
          app.activeQuestion = data.activeQuestion;
...
async function launchQuestion() {
  ...
  app.activeQuestion = {
    ...
    qKey: app.currentCat + "_" + app.currentQIdx + "_" + now // Nano-Unique
  };

  await saveRoomState();
  
  // Wait a bit for cloud propagation before pushing signal
  setTimeout(() => {
    publishState({ type: 'QUESTION_LAUNCH', activeQuestion: app.activeQuestion });
  }, 300);
...
function syncParticipantState() {
  ...
  if (app.activeQuestion) {
    if (app.activeQuestion.qKey !== app.lastQKey) {
      logDebug("New Question Detected: " + app.activeQuestion.qKey);
