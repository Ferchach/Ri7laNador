...
const app = {
  ...
  supTimerInt: null, partTimerInt: null, eventSource: null, selectedOpt: null, isRunning: false, mimeWord: null
};
...
function startListening() {
  ...
  app.eventSource.onmessage = (e) => {
    try {
      ...
      else if (data.type === 'ROOM_RESET') { localStorage.clear(); location.reload(); }
    } catch (err) {}
  };
}
...
async function finishQuestionRound(q) {
  ...
  if (url) {
    ...
    try {
      fetch(url, { method: 'POST', mode: 'no-cors', body: JSON.stringify(logItem) });
    } catch(e) {}
  }
}
...
function nextQ() {
  ...
  if (isLast) {
    if (!confirm("هل تريد إنهاء هذا الصنف؟")) return;
...
async function sendAnswer() {
  const qKey = app.currentCat + "_" + app.currentQIdx;
...
function exportToSheets() {
  ...
  a.download = `musabaka_results_${ROOM_CODE}.csv`; a.click();
}

async function resetCompetition() {
  if (confirm("حذف كل شيء والبدء من جديد؟")) {
...
function showFinalResults() {
  ...
  app.gameLog.forEach((log, idx) => {
    let teamsHtml = "";
    Object.entries(log.teamAnswers).forEach(([tn, ans]) => { teamsHtml += `<div style="font-size:0.8rem; margin-top:4px; padding:4px; border-bottom:1px solid #333;"><strong>فريق ${tn}:</strong> ${ans}</div>`; });
    const d = document.createElement('div'); d.className = 'card card-sm'; d.style.marginBottom = '10px'; d.style.background = 'var(--bg3)';
    d.innerHTML = `<div style="color:var(--gold); font-weight:bold; margin-bottom:5px;">سؤال ${idx+1}: ${log.cat}</div><div style="margin-bottom:8px;">${log.question}</div><div style="color:var(--teal2); font-size:0.9rem; margin-bottom:10px;">✓ الجواب الصحيح: ${log.answer}</div><div style="border-top:1px solid #444; padding-top:5px;">${teamsHtml || '<p class="muted">لا توجد إجابات</p>'}</div>`;
...
window.onload = async () => {
  if (ROOM_CODE) {
    ...
    const ok = await initRoom();
    const role = localStorage.getItem('musabaka_role');
    if (role === 'supervisor') {
      app.role = 'supervisor'; goto('screen-supervisor'); buildCatGrid(); renderScores();
      if (app.currentCat) renderSupQuestion();
