...
// Unique key for this competition. Change this string if you want a separate room.
...
// ══ SHARED STATE (Now Synced via Gun.js) ══
const shared = {
  ...
  timerStart: null,
  duration: 30,
  answers: {}  // key: teamNum, value: answer string
};
...
function checkPin() {
  if ($('sup-pin').value === SUPERVISOR_PIN) {
    ...
    buildCatGrid();
    renderScores();
    listenForAnswers(); // Start listening for incoming answers
  } else {
...
function launchQuestion() {
  ...
  // Clear previous answers in Gun
  db.get('answers').put(null);
  shared.answers = {};

  // Publish state to Gun
  db.get('state').put({
    active: true,
    cat: app.currentCat,
  ...
  app.supTimerInt = setInterval(() => {
    ...
    if (rem <= 0) {
      ...
      db.get('state').get('active').put(false); // End question in Gun
...
function joinTeam() {
  ...
  startRealTimeSync(); // Start listening to Gun
}

function startRealTimeSync() {
  db.get('state').on(data => {
    if (data && data.active) {
      // Sync local shared state from Gun
