// ══ REAL-TIME SYNC (ntfy.sh - Instant Push) ═════════════════
const ROOM_TOPIC = "musabaka_adl_fa180cab_room1"; // Unique topic
...
async function publishState(data) {
  try {
    const res = await fetch(NTFY_URL, {
      ...
      headers: { 'Title': 'Update', 'Tags': 'loudspeaker' }
    });
    return res.ok;
...
const QUESTIONS = {
  ...
  "الجغرافي": [
    ...
    { q: "ما اسم المضيق الفاصل بين المغرب إسبانيا؟", type: "open", ans: "مضيق جبل طارق", pts: 1 },
...
function startListening() {
  ...
  app.eventSource.onmessage = (e) => {
    ...
    } catch (err) { console.error("Sync Msg Error:", err); }
  };
