// ══ CONFIGURATION HYBRIDE (STABLE) ════════════════════════════
let ROOM_CODE = localStorage.getItem('musabaka_room_code') || "";
const KVDB_BUCKET = "https://kvdb.io/mn6xR6f8XzY9vP7u5nB2/"; // Base de données (Aucune limite de taille)
const NTFY_TOPIC = "https://ntfy.sh/musabaka_adl_ping_"; // Signal de réveil ultra-léger
...
async function saveRoomState() {
  ...
  try {
    // 1. Sauvegarde des données lourdes sur KVDB
    await fetch(KVDB_BUCKET + "room_" + ROOM_CODE, {
      method: 'PUT',
      body: JSON.stringify(state)
    });
    // 2. Envoi du petit bip de réveil sur ntfy
    fetch(NTFY_TOPIC + ROOM_CODE, { method: 'POST', body: "SYNC" });
    logDebug("Sauvegarde & Bip envoyés ✅");
  } catch (e) { logDebug("Erreur Sauvegarde ❌"); }
}
...
async function loadRoomState() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      logDebug("Données chargées ✅");
    } else {
      updateSyncStatus('offline', "غرفة جديدة (في انتظار المشرف)");
    }
  } catch(e) {
...
function startSyncListener() {
  ...
  app.eventSource.onopen = () => {
    logDebug("Écouteur de Bip Actif");
    loadRoomState(); // Chargement initial
  };

  app.eventSource.onmessage = (e) => {
    try {
      const envelope = JSON.parse(e.data);
      if (envelope.message === "SYNC") {
        logDebug("Bip Reçu -> Actualisation");
        loadRoomState();
      }
    } catch(err) {}
  };

  // Sécurité: vérifier la base toutes les 3 secondes au cas où le bip est raté
  clearInterval(app.pollInt);
  app.pollInt = setInterval(() => {
    if (app.role === 'participant' || app.role === 'jury') loadRoomState();
  }, 3000);
}
...
const app = {
  ...
  mimeWord: null, activeQuestion: null, lastQKey: null, pollInt: null
};
...
async function confirmReset() {
  if (confirm("هل تريد حذف هذه الغرفة نهائياً من السيرفر؟ سيتم طرد جميع المشاركين.")) {
    ...
    try { await fetch(KVDB_BUCKET + "room_" + ROOM_CODE, { method: 'DELETE' }); } catch(e){}
    localStorage.clear();
    location.reload();
  }
}
