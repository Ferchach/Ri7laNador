// ══ CONFIGURATION ULTRA-RELIABLE ════════════════════════════
let ROOM_CODE = localStorage.getItem('musabaka_room_code') || "";
// Utilisation d'un canal ntfy comme base de données (très robuste)
const SYNC_BASE = "https://ntfy.sh/musabaka_adl_v3_"; 
...
async function saveRoomState() {
  ...
  try {
    // On publie l'état complet sur ntfy
    await fetch(SYNC_BASE + ROOM_CODE, {
      method: 'POST',
      body: JSON.stringify(state),
      headers: { 'Cache': 'yes' } // Demande à ntfy de garder le message en cache
    });
    logDebug("تم الحفظ في السحابة ✅");
    updateSyncStatus('online', "نشط - " + ROOM_CODE);
  } catch (e) { 
    logDebug("خطأ في الحفظ ❌"); 
...
async function loadRoomState() {
  if (!ROOM_CODE) return;
  try {
    // On récupère le DERNIER message posté sur ce canal ntfy
    ...
    // ntfy renvoie une liste, on prend le dernier message valide
    ...
    updateSyncStatus('online', "متصل ✅ - " + ROOM_CODE);
    ...
    logDebug("خطأ اتصال");
    updateSyncStatus('offline', "جاري المحاولة...");
  }
}
...
const app = {
  ...
  supTimerInt: null, partTimerInt: null, selectedOpt: null, isRunning: false, 
  activeQuestion: null, pollInt: null, lastQKey: null
};
...
async function initRoom() {
  ...
  app.pollInt = setInterval(loadRoomState, 2000); 
}
