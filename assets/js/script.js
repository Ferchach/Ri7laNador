// ============================================================================
// MUSABAKA THAQAFIYA - ARCHITECTURE V8 (RÉÉCRITURE TOTALE & SÉCURISÉE)
// ============================================================================

const TOPIC_URL = "https://ntfy.sh/musabaka_v8_"; 
...
async function saveRoomState() {
  ...
  try {
    ...
    const res = await fetch(TOPIC_URL + ROOM_CODE, { method: 'POST', body: payload });
    if (!res.ok) throw new Error("HTTP " + res.status);
    if (app.role === 'supervisor') showToast("✅ تم حفظ وإرسال البيانات");
  } catch(e) {
    if (app.role === 'supervisor') showToast("❌ خطأ في الإرسال: تأكد من الإنترنت", true);
  }
}

function startSyncListener() {
  if (app.eventSource) app.eventSource.close();
  app.eventSource = new EventSource(TOPIC_URL + ROOM_CODE + "/sse");
  
  app.eventSource.onopen = () => updateSyncStatus('online', "متصل ✅");
  ...
  app.eventSource.onerror = () => updateSyncStatus('offline', "جاري الاتصال...");
}

async function forceSync() {
  if (!ROOM_CODE) return;
  try {
    const res = await fetch(TOPIC_URL + ROOM_CODE + "/json?poll=1");
...
async function initRoom() {
  ...
  startSyncListener(); forceSync(); goto('screen-role');
}
...
async function confirmReset() {
  if (confirm("هل تريد حذف هذه الغرفة نهائياً؟ سيتم طرد جميع المشاركين.")) {
    ...
    app.currentCat = null; app.currentQIdx = 0; app.activeQuestion = null;
    localStorage.clear(); await saveRoomState(); location.reload();
  }
}
...
function syncParticipantState() {
  ...
  if (app.activeQuestion) {
    ...
    app.lastQKey = null; qViewDiv.classList.add('hidden'); waitingDiv.classList.remove('hidden');
...
function renderPartOpts(data) {
  ...
  if (data.qType === 'mcq') {
    area.innerHTML = '<div class="options-grid" id="part-mcq-grid"></div>';
    data.opts.forEach(o => {
      const b = document.createElement('div'); b.className = 'opt-btn'; b.textContent = o;
      b.onclick = () => { document.querySelectorAll('#part-mcq-grid .opt-btn').forEach(x=>x.classList.remove('selected')); b.classList.add('selected'); app.selectedOpt = o; };
      $('part-mcq-grid').appendChild(b);
    });
  } else {
...
async function sendAnswer(ansParam) {
  ...
  shared.answers[app.teamNum] = ans; await saveRoomState();
  $('part-send-btn').classList.add('hidden'); $('part-sent-msg').classList.remove('hidden');
  clearInterval(app.partTimerInt);
}
