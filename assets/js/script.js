// ══ REAL-TIME SYNC (ntfy.sh - Ultra Robust) ═════════════════
const ROOM_TOPIC = "musabaka_adl_youssef_2026_final"; 
...
async function publishState(data) {
  try {
    ...
    logDebug("Broadcast status: " + res.status);
    return res.ok;
  } catch (e) { 
...
function startListening() {
  ...
  logDebug("Connecting to: " + ROOM_TOPIC);
  ...
  app.eventSource.onmessage = (e) => {
    try {
      ...
      if (typeof data === 'string') {
        try { data = JSON.parse(data); } catch(pErr) { return; }
      }
      
      logDebug("Signal received: " + data.type);
      ...
      logDebug("Recv Error: " + err.message);
    }
  };

  app.eventSource.onerror = () => {
    logDebug("Connection lost, retrying...");
    updateSyncStatus('offline', "إعادة اتصال...");
  };
}

function handleIncomingQuestion(data) {
  if (app.role !== 'participant') return;
  logDebug("Showing Question...");
...
async function launchQuestion() {
  ...
  if (!ok) { alert("خطأ في الإرسال!"); return; }
